...
# driver ids
drive_id <- d_all %>% filter(role == "team") %>% select(uid) %>% unique()
...
reliability_driver <- bind_rows(res, ipip, fluid, conf, bias, discrim, switch_errors, switch_time, switch_cost, wm_acc, flank_errors, flank_time, flank_cost) %>% 
    rename(a_driver = rel)
    # write_csv("output/reliability_driver.csv")
